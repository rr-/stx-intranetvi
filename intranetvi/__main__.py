...
from intranetvi.api import IntranetApi, Worklog
...
def parse_args() -> configargparse.Namespace:
    parser = configargparse.ArgumentParser(
        prog="intranetvi",
        default_config_files=[str(CONFIG_DIR / "intranetvi.ini")],
    )

    parser.add_argument(
        "--user-id", required=True, type=int, help="Intranet API user ID"
    )
    parser.add_argument(
        "--session-id", required=True, help="Intranet API session token"
    )
    parser.add_argument(
        "--prolog",
        type=Path,
...
def dump_worklogs(
    ...
    columns = ["id", "duration", "project_id", "ticket", "description"]
    ...
    for day in range((end_date - start_date).days + 1):
        ...
        dump_worklog_day(
            ...
            rows=[
                [
                    str(getattr(worklog, column))
                    if getattr(worklog, column) is not None
                    else ""
...
def apply_diff(api: IntranetApi, diff: WorklogDiff) -> None:
...
def run_editor_and_apply_diff(
    api: IntranetApi, source_worklogs: T.List[Worklog], path: Path
) -> None:
...
def main() -> None:
    args = parse_args()
    api = IntranetApi(args.user_id, args.session_id)
